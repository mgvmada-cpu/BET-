...
import {
  ...
  Save, PlayCircle, CheckCircle2, XCircle, Radio
} from "lucide-react";
import {
...
function LiveFeedView({ t, lang, onAnalyze, liveMatches, apiState }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: COLORS.textLow, fontSize: 12.5, marginTop: -8, marginBottom: 6 }}>{t.todayMatchesSub}</p>
      {apiState === "mock" && (
...
export default function SportsBettingDashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchOdds() {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          // Réponse vide (pas de tournoi actif, ou clé absente côté serveur) → mock
    ...
    fetchOdds();
    // Rafraîchissement toutes les 3 minutes pour ménager le quota de crédits de l'API
    const id = setInterval(fetchOdds, 3 * 60 * 1000);
    return () => {
      cancelled = true;
      clearInterval(id);
    };
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100%", fontFamily: "Inter, sans-serif", color: COLORS.textHi }}>
      ...
      <div style={{ padding: "20px 20px 60px" }}>
        {tab === "live" && <LiveFeedView t={t} lang={lang} onAnalyze={handleAnalyze} liveMatches={liveMatches} apiState={apiState} />}
