...
const T = {
  fr: {
    ...
    loadRealData: "Charger les données réelles",
    loadingRealData: "Chargement…",
    realDataLoaded: "Données réelles chargées (stats service/retour ajustées)",
    realDataError: "Impossible de charger les données réelles — vérifie la clé RAPIDAPI_KEY ou l'orthographe des noms",
    realRanking: "Classement réel",
    realRankingUnavailable: "Classement non trouvé pour ces noms — vérifie l'orthographe exacte",
    realStatsUnavailable: "Stats service/retour non trouvées dans la réponse — curseurs laissés tels quels",
    serveShort: "Service",
    returnShort: "Retour",
    loadHistory: "Voir l'historique des matchs",
    loadingHistory: "Chargement de l'historique…",
    previousMatch: "Match précédent",
    vs: "vs",
    historyUnavailable: "Historique non disponible pour ce joueur",
  ...
  en: {
    ...
    loadRealData: "Load real data",
    loadingRealData: "Loading…",
    realDataLoaded: "Real data loaded (serve/return stats adjusted)",
    realDataError: "Couldn't load real data — check RAPIDAPI_KEY or the player names",
    realRanking: "Real ranking",
    realRankingUnavailable: "No ranking match for these names — check the exact spelling",
    realStatsUnavailable: "Serve/return stats not found in the response — sliders left unchanged",
    serveShort: "Serve",
    returnShort: "Return",
    loadHistory: "View match history",
    loadingHistory: "Loading history…",
    previousMatch: "Previous match",
    vs: "vs",
    historyUnavailable: "History unavailable for this player",
...
function TennisView({ t, lang, form, setForm, onSave }) {
  ...
  const [realState, setRealState] = useState("idle"); // idle | ok | error
  const [realInfo, setRealInfo] = useState(null);

  async function loadRealData() {
    if (!form.playerA || !form.playerB) return;
    setLoadingReal(true);
    setRealState("idle");
    try {
      const params = new URLSearchParams({
        playerA: form.playerA, playerB: form.playerB, tour: form.tour || "atp",
      });
      if (form.surface) params.set("surface", form.surface);
      const res = await fetch(`/api/tennis-analysis?${params.toString()}`);
      if (!res.ok) throw new Error(String(res.status));
      const data = await res.json();
      const updates = {};
      if (data.serveA != null) updates.serveA = data.serveA;
      if (data.serveB != null) updates.serveB = data.serveB;
      if (data.returnA != null) updates.returnA = data.returnA;
      if (data.returnB != null) updates.returnB = data.returnB;
      if (Object.keys(updates).length > 0) setForm({ ...form, ...updates });
      setRealInfo(data);
      setRealState("ok");
    } catch (e) {
      setRealState("error");
  ...
  const [historyState, setHistoryState] = useState("idle"); // idle | ok | error
  ...
  async function loadHistory() {
    ...
    try {
      const [resA, resB] = await Promise.all([
        fetch(`/api/player-history?player=${encodeURIComponent(form.playerA)}`),
        fetch(`/api/player-history?player=${encodeURIComponent(form.playerB)}`),
      ]);
    ...
    <div className="two-col">
      <Card>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div>
            <span style={{ fontSize: 11, color: COLORS.textLow, textTransform: "uppercase", letterSpacing: "0.06em" }}>{t.tour}</span>
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              {["atp", "wta"].map((tr) => (
                <button key={tr} onClick={() => setForm({ ...form, tour: tr })}
                  style={{
                    flex: 1, padding: "7px", borderRadius: 7, fontSize: 12, cursor: "pointer",
                    border: `1px solid ${(form.tour || "atp") === tr ? COLORS.accent : COLORS.border}`,
                    background: (form.tour || "atp") === tr ? COLORS.accentSoft : COLORS.bgAlt,
                    color: (form.tour || "atp") === tr ? "#C7C9FF" : COLORS.textMid,
                  }}>
                  {tr.toUpperCase()}
                </button>
              ))}
          ...
          <button
            ...
            style={{
              ...
              background: COLORS.accentSoft, border: `1px solid ${COLORS.accent}55`, color: "#C7C9FF",
              borderRadius: 9, padding: "9px", fontWeight: 600, fontSize: 12.5,
              cursor: loadingReal ? "default" : "pointer", opacity: loadingReal ? 0.6 : 1,
            }}
          >
            <RefreshCw size={14} style={loadingReal ? { animation: "spin 0.9s linear infinite" } : {}} />
            {loadingReal ? t.loadingRealData : t.loadRealData}
          </button>
          {realState === "ok" && realInfo && (
            <div style={{ background: COLORS.bgAlt, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 9, padding: 10, fontSize: 11.5, color: COLORS.textMid, display: "grid", gap: 5 }}>
              <div style={{ color: COLORS.positive, fontWeight: 600 }}>{t.realDataLoaded}</div>
              {(realInfo.rankA || realInfo.rankB) ? (
                <div>
                  {t.realRanking}: <strong style={{ color: COLORS.textHi }}>{form.playerA} #{realInfo.rankA ?? "?"}</strong> · <strong style={{ color: COLORS.textHi }}>{form.playerB} #{realInfo.rankB ?? "?"}</strong>
                  {(realInfo.pointsA || realInfo.pointsB) && (
                    <> ({realInfo.pointsA ?? "?"} pts / {realInfo.pointsB ?? "?"} pts)</>
                  )}
                </div>
              ) : (
                <div style={{ color: COLORS.textLow }}>{t.realRankingUnavailable}</div>
              )}
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                <span>{form.playerA}: {t.serveShort} {form.serveA}% · {t.returnShort} {form.returnA}%</span>
                <span>{form.playerB}: {t.serveShort} {form.serveB}% · {t.returnShort} {form.returnB}%</span>
              </div>
              {realInfo.serveA == null && realInfo.serveB == null && realInfo.returnA == null && realInfo.returnB == null && (
                <div style={{ color: COLORS.amber }}>{t.realStatsUnavailable}</div>
              )}
            </div>
          )}
          {realState === "error" && (
            <div style={{ color: COLORS.negative, fontSize: 11.5 }}>{t.realDataError}</div>
          )}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 9, padding: "9px", fontWeight: 600, fontSize: 12.5,
              cursor: loadingHistory ? "default" : "pointer", opacity: loadingHistory ? 0.6 : 1,
            }}
          >
            <RefreshCw size={14} style={loadingHistory ? { animation: "spin 0.9s linear infinite" } : {}} />
            {loadingHistory ? t.loadingHistory : t.loadHistory}
          </button>

          {historyState === "ok" && (historyA || historyB) && (
            <div style={{ display: "grid", gap: 10 }}>
              {[{ label: form.playerA || t.playerA, data: historyA }, { label: form.playerB || t.playerB, data: historyB }].map((p, i) => (
                <div key={i} style={{ background: COLORS.bgAlt, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 9, padding: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.textHi, marginBottom: 6 }}>{p.label}</div>
                  {p.data && p.data.mostRecent ? (
                    <div style={{ fontSize: 11.5, color: COLORS.textMid, marginBottom: 6 }}>
                      <span style={{ color: COLORS.accent, fontWeight: 600 }}>{t.previousMatch}: </span>
                      {t.vs} {p.data.mostRecent.opponent || "?"} — {p.data.mostRecent.score || "?"}
                      {p.data.mostRecent.tournament && <> ({p.data.mostRecent.tournament})</>}
                      {p.data.mostRecent.date && <> · {p.data.mostRecent.date}</>}
                    </div>
                  ) : (
                    <div style={{ fontSize: 11.5, color: COLORS.textLow, marginBottom: 6 }}>{t.historyUnavailable}</div>
                  )}
                  {p.data && p.data.matches && p.data.matches.length > 1 && (
                    <div style={{ display: "grid", gap: 3, fontSize: 11, color: COLORS.textLow, fontFamily: "'JetBrains Mono', monospace" }}>
                      {p.data.matches.slice(1, 5).map((m, j) => (
                        <div key={j}>{t.vs} {m.opponent || "?"} — {m.score || "?"}{m.date && ` (${m.date})`}</div>
                      ))}
                    </div>
                  )}
                </div>
          ...
          {historyState === "error" && (
            <div style={{ color: COLORS.negative, fontSize: 11.5 }}>{t.historyError}</div>
          )}
...
export default function SportsBettingDashboard() {
  ...
  const [tennisForm, setTennisForm] = useState({
    ...
    surface: "hard", format: "bo3", bankroll: 1000, tour: "atp",
  ...
  function handleAnalyze(match) {
    if (match.sport === "tennis") {
      setTennisForm((f) => ({ ...f, playerA: match.playerA, playerB: match.playerB, oddsA: match.oddsA, oddsB: match.oddsB, surface: match.surface || f.surface, tour: (match.tour || "ATP").toLowerCase() }));
      setTab("tennis");
    } else {
