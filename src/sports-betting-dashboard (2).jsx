      ...
      <Card>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {realState === "ok" && realInfo && (
            <div style={{ background: COLORS.bgAlt, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 9, padding: 10, fontSize: 11.5, color: COLORS.textMid }}>
              <div style={{ color: COLORS.positive, fontWeight: 600, marginBottom: 4 }}>{t.realDataLoaded}</div>
              {(realInfo.rankA || realInfo.rankB) && (
                <div>{t.realRanking}: {form.playerA} #{realInfo.rankA ?? "?"} · {form.playerB} #{realInfo.rankB ?? "?"}</div>
              )}
            </div>
          )}
          {realState === "error" && (
